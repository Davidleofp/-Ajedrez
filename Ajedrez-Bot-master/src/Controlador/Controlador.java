...
import Vista.VentanaEleccionFichaNegra;
...
public class Controlador implements ActionListener {
    ...
    private int segundosTranscurridosA;
    private int segundosTranscurridosB;
    ...
    public Controlador() {
        ...
        vista.getCasillaNegra().setVisible(false);
        vista.getCasillaBlanca().setVisible(true);
    }
    ...
    private void añadirActionEvents() {
        ...
        vista.jMenuItem3.addActionListener(this);
        vista.jMenuItem4.addActionListener(this);
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        if (ae.getActionCommand().equals("CREAR")){
            ...
            this.vista.jMenuItem3.setEnabled(false);
            this.vista.jMenuItem4.setEnabled(false);

        }else if(ae.getActionCommand().equals("UNIRSE")){
            ...
            if (turnoJugador == 'A') {
                ...
                if (comprobarSiLaFichaEsBlanca(posicionActual)) {
                    ...
                    if (movimientos.esPosibleEsteMovimiento(tablero, posicionAntigua, posicionNueva)){
                        ...
                        detenerRelojReyBlanco();
                        turnoJugador = 'B';
                        comprobarJaqueMateHaciaNegras();
                        vista.getCasillaBlanca().setVisible(false);
                        vista.getCasillaNegra().setVisible(true);
                    }
                }
            }
            if (turnoJugador == 'B') {
                iniciarRelojReyNegro();
                posicionActual = getBotonPosicionString(ae.getSource());
        ...
        for(int i = 0;i<8;i++){
            for(int j = 0;j<8;j++){
                ...
                if(comprobarSiLaFichaEsNegra(posicion)){
                    String[] movimientosF = movimientos.movimientosBmodificados(tablero, posicion);
                    if(!movimientosF[0].equals("")){
                        jaqueMate = false;
                        break;
                    }
    ...
    private void eleccionDePeonNegro() {
        VentanaEleccionFichaNegra ventanaElec = new VentanaEleccionFichaNegra(null, true);
        ventanaElec.setVisible(true);
    }
    ...
    private void iniciarRelojReyNegro() {
        if (timerB == null || !timerB.isRunning()) {
        // Solo inicia el temporizador si no está en funcionamiento
        timerB = new Timer(1000, new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                segundosTranscurridosB++;
                actualizarRelojReyNegro();
            }
        });
        timerB.start(); // Inicia el temporizador
        }
    }
    private void detenerRelojReyNegro() {
    if (timerB != null && timerB.isRunning()) {
        timerB.stop(); // Detiene el temporizador si está en ejecución
        }
    }
    
    private void actualizarRelojReyNegro() {
        int minutos = segundosTranscurridosB / 60;
        int segundos = segundosTranscurridosB % 60;
        String tiempoB = String.format("%02d:%02d", minutos, segundos);
        vista.getRelogNegro().setText("TIEMPO DE JUEGO REY NEGRO: " + tiempoB);
    }
    
    private void iniciarRelojReyBlanco() {
        if (timerA == null || !timerA.isRunning()) {
        // Solo inicia el temporizador si no está en funcionamiento
            timerA = new Timer(1000, new ActionListener() {
            @Override
                public void actionPerformed(ActionEvent e) {
                    segundosTranscurridosA++;
                    actualizarRelojReyBlanco();
                }
            });
        timerA.start(); // Inicia el temporizador
        }

    }
    
    private void detenerRelojReyBlanco() {
    if (timerA != null && timerA.isRunning()) {
        timerA.stop(); // Detiene el temporizador si está en ejecución
        }
    }
    
    private void actualizarRelojReyBlanco() {
        int minutos = segundosTranscurridosA / 60;
        int segundos = segundosTranscurridosA % 60;
        String tiempoA = String.format("%02d:%02d", minutos, segundos);
        vista.getRelogBlanco().setText("TIEMPO DE JUEGO REY BLANCO: " + tiempoA);
    }
